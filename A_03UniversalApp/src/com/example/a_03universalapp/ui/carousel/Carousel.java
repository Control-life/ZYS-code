 ...
 */
public class Carousel extends CarouselSpinner implements GestureDetector.OnGestureListener,CarouselAdapter.OnItemClickListener {
...
//	private boolean mShouldCallbackOnUnselectedItemClick = true;
	...
	//	TypedArray images = getResources().obtainTypedArray(R.array.entries1);
	...
	protected int getChildDrawingOrder(int childCount, int i) {
		...
		//System.out.println("getChildDrawingOrder:" + i);
		//System.out.println("@@@@@@@@@@@@@@@@@@@@@@");
		//for (int j = 0;j < childCount;j++) {
		//	System.out.println("see :"+sl.get(j).getIndex());
		//}
		//System.out.println("*************************");
		Collections.sort(sl);
	//	for (int j = 0;j < childCount;j++) {
		//	System.out.println("lokk :"+sl.get(j).getIndex());
		//}
		//System.out.println("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$");
...
//		}
		...
		public void startUsingDistance(float deltaAngle) {
			...
			System.out.println("startUsingDistance");
	...
	private class ImageAdapter extends BaseAdapter {
		...
		public void SetImages(TypedArray array, TypedArray names, boolean reflected) {
			...
			Drawable[] namesDrawable = new Drawable[names.length()];
			...
			for (int i = 0;i < array.length();i++) {
				...
				namesDrawable[i] = names.getDrawable(i);
				
				Bitmap originalImage = ((BitmapDrawable) drawables[i]).getBitmap();
				Bitmap nameImage = ((BitmapDrawable)namesDrawable[i]).getBitmap();
				...
				if (bmpWidth > IMAGE_MAX_WIDTH || bmpHeight > IMAGE_MAX_WIDTH) {
					...
					//originalImage =((BitmapDrawable) ((AppInfo)mlistAppInfo.get(i)).getAppIcon()).getBitmap();
				}
				if (reflected) {
		...
		public View getView(final int position, View convertView, ViewGroup parent) {
			mImages[position].setOnClickListener(new View.OnClickListener() {
				
				@Override
				public void onClick(View v) {
					Message msg = Message.obtain();
					msg.obj = mImages[position];
					msg.what = 1;
				}
			});
	...
	private void makeAndAddView(int position, float angleOffset) {
		System.out.println("makeAndAddView");
		CarouselItem child;
...
//	}
	...
	void trackMotionScroll(float deltaAngle) {
		...
		System.out.println("trackMotionScroll");
		if (getChildCount() == 0) {
			return;
		}
